...
function UpdateMarket(message)
{
 ...
 // lastupdated = Date.now();
  ...
  for (var i=0; i<message.coins.length; i++)
  {
    ...
    const price = (message.coins[i].fromBuyerToSeller/(message.coins[i].volume == 0 ? 1 : message.coins[i].volume)).toFixed(8);
...
function UpdateTradeHistory(history)
{
  ...
  for (var i=0; i<history.length; i++)
  {
    ...
    const tr = $('<tr></tr>')
      ...
      .append($('<td>'+volume+'</td>'))
      .append($('<td>'+(price*1.0).toFixed(8)+'</td>'));
...
function UpdateOrders(orders)
{
  ...
  {
    const price = utils.MakePrice(orders.buy[i].price);
    const amountMain = utils.MakePrice(orders.buy[i].price*orders.buy[i].amount); 
    const amountPair = utils.MakePrice(orders.buy[i].amount);
  ...
  {
    const price = utils.MakePrice(orders.sell[i].price); 
    const amountMain = utils.MakePrice(orders.sell[i].price*orders.sell[i].amount); 
    const amountPair = utils.MakePrice(orders.sell[i].amount); 
  ...
  const askButton = $('<button id="button_max_ask" type="button" class="p-0 btn btn-link"></button>').append((txtBuyPrice*1.0).toFixed(8)).on('click', e => {
        $('#inputBuyPrice').val(txtBuyPrice);
      })
  const bidButton = $('<button id="button_max_bid"type="button" class="p-0 btn btn-link"></button>').append((txtSellPrice*1.0).toFixed(8)).on('click', e => {
        $('#inputSellPrice').val(txtSellPrice);
      })
  
  $('#id_max_bid').empty().append(bidButton); 
  $('#id_max_ask').empty().append(askButton); 
...
function UpdateUserOrders(userOrders)
{
  ...
  for (var i=0; i<userOrders.length; i++)
  {
    ...
    const tr = $('<tr></tr>')
      ...
      .append($('<td>'+(userOrders[i].amount*1.0).toFixed(8)+' '+coinNameToTicker[g_CurrentPair].ticker+'</td>'))
      .append($('<td>'+(userOrders[i].price*1.0).toFixed(8)+" "+coinNameToTicker[utils.MAIN_COIN].ticker+'</td>'))
...
function UpdateBalance(message)
{
  ...
  if (message.coin && (message.balance != undefined))
  {
    if (unescape(message.coin.name) == utils.MAIN_COIN)
    {
      ...
      buyBalance = (message.balance*1.0).toFixed(8);
    ...
    if (unescape(message.coin.name) == g_CurrentPair)
    {
      ...
      sellBalance = (message.balance*1.0).toFixed(8);
...
function UpdateBuyComissionFromTotal()
{
  ...
  try 
  {
    const amount = (total / (price + utils.COMISSION*price)).toFixed(8);
    const comission = utils.COMISSION*amount*price;
    $('#inputBuyComission').val(comission.toFixed(8));
